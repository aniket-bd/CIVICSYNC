  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{
        ...
        <div style={{ background: '#f5f5f7', border: '1px solid rgba(0, 0, 0, 0.06)', borderRadius: '12px', padding: '10px' }}>
          <div style={{ fontSize: '11px', color: '#86868b' }}>Total Ingested</div>
          <div style={{ fontSize: '18px', fontWeight: 700, color: '#1d1d1f' }}>{report.totalRecords}</div>
        </div>

        <div style={{ background: 'rgba(26, 127, 55, 0.08)', border: '1px solid rgba(26, 127, 55, 0.22)', borderRadius: '12px', padding: '10px' }}>
          <div style={{ fontSize: '11px', color: '#1a7f37', display: 'flex', alignItems: 'center', gap: '4px' }}>
            <CheckCircle2 size={12} /> Valid Records
          </div>
          <div style={{ fontSize: '18px', fontWeight: 700, color: '#1a7f37' }}>{report.validRecordsCount}</div>
        </div>

        <div style={{ background: 'rgba(0, 0, 0, 0.04)', border: '1px solid rgba(0, 0, 0, 0.08)', borderRadius: '12px', padding: '10px' }}>
          <div style={{ fontSize: '11px', color: '#6e6e73', display: 'flex', alignItems: 'center', gap: '4px' }}>
            <AlertTriangle size={12} /> Missing Fields
          </div>
          <div style={{ fontSize: '18px', fontWeight: 700, color: '#6e6e73' }}>{report.missingFieldsCount}</div>
        </div>
        ...
        <div style={{ background: 'rgba(0, 0, 0, 0.04)', border: '1px solid rgba(0, 0, 0, 0.08)', borderRadius: '12px', padding: '10px' }}>
          ...
          <div style={{ fontSize: '18px', fontWeight: 700, color: '#86868b' }}>{report.duplicateRecordsCount}</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left' }}>
          <thead>
            <tr style={{ background: '#f5f5f7', color: '#86868b', borderBottom: '1px solid rgba(0, 0, 0, 0.06)' }}>
              ...
              return (
                <tr
                  ...
                  style={{
                    ...
                    background: item.selectedForImport ? 'rgba(0, 113, 227, 0.05)' : 'transparent'
                  }}
                >
                  ...
                  <td style={{ padding: '10px', fontFamily: 'SF Mono, ui-monospace, monospace', color: '#0071e3' }}>
                    {p?.tenderNumber || 'N/A'}
                  </td>
                  <td style={{ padding: '10px', maxWidth: '240px', fontWeight: 600, color: '#1d1d1f' }}>
                  ...
                  <td style={{ padding: '10px', fontFamily: 'SF Mono, ui-monospace, monospace', color: '#1a7f37' }}>
                    {formatINR(p?.budget)}
                  </td>
                  <td style={{ padding: '10px', fontFamily: 'SF Mono, ui-monospace, monospace' }}>
                    {p?.depthMeters ? `${p.depthMeters} m` : 'N/A'}
                  </td>
                  <td style={{ padding: '10px' }}>
                    {item.isDuplicate ? (
                      <span style={{ color: '#6e6e73', display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '11px' }}>
                      ...
                      <span style={{ color: '#1a7f37', display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '11px' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '13px', color: '#86868b' }}>
          Selected <strong style={{ color: '#0071e3' }}>{selectedCount}</strong> of {items.length} records for database commit
        </div>
