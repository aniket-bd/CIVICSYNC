...
export const Procedural3DViewer: React.FC<Procedural3DViewerProps> = ({
  ...
  useEffect(() => {
    ...
    loadTenderMapTexture({
      ...
      style: mapBasemap,
      project,   // enables auto-zoom to match project footprint
    }).then(({ texture, bounds }) => {
      if (cancelled) {
        texture.dispose();
        return;
  ...
  useEffect(() => {
    ...
    // Use loaded tile bounds if available; otherwise derive from project geometry
    const bounds: MapTileBounds = mapBoundsRef.current ?? deriveProjectBounds(project);

    const metrics = getProjectPolygonMetrics(project, bounds);
    const dims = getCalibratedProjectDimensions(project, metrics);

    // Target polygon centroid; auto-adjust camera radius based on polygon size
    const polyDiag = Math.sqrt(metrics.spanX * metrics.spanX + metrics.spanZ * metrics.spanZ);
    const desiredRadius = Math.max(18, Math.min(110, polyDiag * 2.8));
    sph.current.radius = desiredRadius;
  ...
  const setCamPreset = (p: 'iso' | 'top' | 'cut' | 'street') => {
    const curBounds: MapTileBounds = mapBoundsRef.current ?? deriveProjectBounds(project);
  ...
  const currentBounds: MapTileBounds = mapBoundsRef.current ?? deriveProjectBounds(project);
