...
export const MapLibre2D: React.FC<MapLibre2DProps> = ({
  ...
  useEffect(() => {
    ...
    visibleProjects.forEach(project => {
      ...
      el.style.boxShadow = isSelected ? '0 0 20px rgba(0, 113, 227, 0.45), 0 4px 12px rgba(0,0,0,0.3)' : '0 2px 8px rgba(0,0,0,0.3)';
      ...
      const popupHtml = `
        <div style="padding: 12px; font-family: -apple-system, sans-serif; min-width: 220px; color: #1d1d1f; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(20px); border-radius: 12px; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.06); border: 1px solid rgba(0, 0, 0, 0.08);">
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', minHeight: '440px', overflow: 'hidden', fontFamily: F }}>
      ...
      <div style={{
        ...
        gap: '8px',
        zIndex: 10,
        boxShadow: '0 8px 24px -4px rgba(0, 113, 227, 0.10)'
      }}>
      ...
      {selectedProject && (
        <div style={{
          ...
          padding: '16px 20px',
          zIndex: 10,
          boxShadow: '0 16px 40px -4px rgba(0, 0, 0, 0.08), 0 4px 16px rgba(0,0,0,0.08)',
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px', flexWrap: 'wrap' }}>
                ...
                {selectedProject.gpr && (
                  <span style={{ fontSize: '10.5px', background: 'rgba(0, 113, 227, 0.08)', color: '#0071e3', border: '1px solid rgba(0, 0, 0, 0.07)', padding: '2px 6px', borderRadius: '6px', display: 'inline-flex', alignItems: 'center', gap: '3px', fontWeight: 600 }}>
                    <Radar size={11} /> GPR
                  </span>
                )}
              </div>
              ...
              {hasCoordinates ? (
                <div style={{ fontSize: '12px', color: '#515154', marginTop: '3px' }}>
                  {selectedProject.locationName} · Depth: <strong style={{ color: '#0071e3' }}>{formatDepth(selectedProject.depthMeters)}</strong>
                </div>
              ) : (
